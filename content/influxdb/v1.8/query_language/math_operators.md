...
* [数学运算符](#mathematical-operators)
  * [加法 (Addition)](#subtraction)
  * [减法 (Subtraction)](#subtraction)
  * [乘法 (Multiplication)](#multiplication)
  * [除法 (Division)](#division)
  * [模 (Modulo)](#modulo)
  * [按位AND (Bitwise AND)](#bitwise-and)
  * [按位OR (Bitwise OR)](#bitwise-or)
  * [按位异或 (Bitwise Exclusive-OR)](#bitwise-exclusive-or)
  * [数学运算符的常见问题](#common-issues-with-mathematical-operators)
* [不支持的运算符](#unsupported-operators)
...
InfluxDB不支持在SELECT子句中将数学运算与通配符(`*`)或[正则表达式](/influxdb/v1.8/query_language/explore-data/#regular-expressions)结合使用。以下查询是无效的，系统会返回错误：
...
目前不支持在函数内使用数学运算。请注意，InfluxDB只允许在`SELECT`子句中使用函数。
...
```
> InfluxQL支持[子查询](/influxdb/v1.8/query_language/explore-data/#subqueries)，它提供与在函数内使用数学运算符类似的功能。请查阅[数据探索](/influxdb/v1.8/query_language/explore-data/#subqueries)相关章节获得更多信息。

## 不支持的运算符

### 等式和不等式

对所有类型的数据，在`SELECT`子句中使用`=`、`!=`、`<`、`>`、`<=`、`>=`或`<>`会返回空的结果。请查看GitHub问题[3525](https://github.com/influxdb/influxdb/issues/3525)。

### 逻辑运算符

使用`!|`、`NAND`、`XOR`或`NOR`会产生解析错误。

另外，在查询的`SELECT`子句中使用`AND`或者`OR`并不会跟使用数学运算符的效果一样，只会产生空的结果，因为`AND`和`OR`都是InfluxQL中的标记(tokens)。但是，您可以对布尔类型的数据使用按位操作符：`&`、`|`和`^`

### Bitwise Not

没有bitwise not运算符，因为您期望的结果依赖您的bitfield的宽度。InfluxQL不知道您的bitfield的宽度，所以无法实现一个合适的bitwise not运算符。
